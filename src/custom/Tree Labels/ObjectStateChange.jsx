...
export default function ObjectStateChange({ data, path, key, videoRef, setPlaying, videoOffset = 0 }) {
    ...
    return <span><b>{data['object_of_change']}</b>: [pre: {Math.round(data.pre_frame?.video_frame.frame_number / 30.0)}s, pnr: {Math.round(data.pnr_frame?.video_frame.frame_number / 30.0)}s, post: {Math.round(data.post_frame?.video_frame.frame_number / 30.0)}s]</span >
}
